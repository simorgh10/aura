...
- **Action Taken (Follow-up 3):** Developed reactive root overlap constraint check & push logic, and added "Auto-Align / Rebalance Graph" force layout reset.
- **Architectural Decision:** Implemented an iterative overlap-resolution pass on top-level root containers in the calculated layout signal, pushing colliding parent boxes apart. Added `rebalanceLayout()` method to reset all custom drag offsets and center the infinite viewport, naturally redistributing all components in a perfectly spatially balanced spring-embedder layout structure.
